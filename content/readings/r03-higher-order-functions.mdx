...
// but then does nothing with it so it is never called. 
// Since it is also not stored anywhere, the value of the expression
// (i.e., the new function) just dissapears.

let doubleNumber = (num: number): number => {
    return num * 2;
}
// Better! Now the view of the arrow expression (i.e., the new function) 
// is stored in the variable doubleNumber. Using this variable,
// we can refer to this function definition and call it from elsewhere.
```
...
```ts title="Arrow Function"
/** Function that doubles the input number */
// Here we do not have a type annocation for the variable doubleNumber
// The type is being inferred as (num: number) => number
let doubleNumber = (num: number): number => {
    return num * 2;
}
...
/** Function that doubles the input number */
// Here we are providing the type annocation for teh variable
// doubleNumber explicitly.
